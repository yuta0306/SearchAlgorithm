from typing import AnyStr, Dict, Text, Union, List
import sys
...
class Dijkstra:
    def __init__(self, graph: Dict[Union[str, int], Union[Dict[str, List[str or int or float]]]],
                start: Union[str, int]=None, goal: Union[str, int]=None) -> None:
        self._validate_graph(graph=graph)
        self.graph = graph
        self.OPEN = list()
        self.CLOSED = list()
        self.PTR = dict()
    ...
    def _validate_graph(self, graph: dict):
        for _, data in graph.items():
            nodes = data.get('node', [])
            costs = data.get('cost', [])
            if len(nodes) != len(costs):
                raise ValueError('The size of node and that of cost are different.')